...
writer = SummaryWriter('/data/datasets/starganv2/celeba/runs/')
...
class Solver(object):
    ...
    def train_generator(self, x_real, g_x_fake, g_s_tilde_trg, label_org, label_trg):
        """
        train generator
        :param x_real:
        :param label_org:
        ...
        s_hat_org = torch.squeeze(torch.stack([torch.index_select(x, 0, i) for x, i in
                                               zip(torch.chunk(torch.stack(s_hat_cyc, 1), chunks=self.num_domains,
                                                               dim=1),
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_source, label_source) in enumerate(self.data_loader):
                data_iter = iter(self.data_loader)
                # x_source, label_source = next(data_iter)
                x_reference, label_ref = next(data_iter)
                # Prepare input images and target domain labels.
                x_source = x_source.to(self.device)
                x_reference = x_reference.to(self.device)
                label_ref = label_ref.to(self.device)
                # c_trg_list = self.create_labels(c_org, self.c_dim, self.dataset, self.selected_attrs)

                # Translate images.
                x_fake_list = [x_source, x_reference]
                random_domains = torch.randperm(self.num_domains).to(self.device)

                # Get style code from reference image
                for d in range(self.num_domains):
                    domain = torch.FloatTensor([d] * self.batch_size).to(self.device)
                    ref_style = self.get_reference_style(x_reference, domain)
                    x_fake_list.append(self.G(x_source, ref_style))

                    # Get style code from latent code
                    latent_style = self.generate_style_code(domain)
